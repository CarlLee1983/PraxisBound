...
* R2: Step rules: no `run` without an earlier exit-0 `run-dry-run`;
  ...
  `work-add`; every step but the last exits 0, except that a non-zero
  `work-list` may be followed immediately by `goal-create` (contract §11
  step 3, amended by Human Review 2026-09-24); an exit-0 `work-add` has
