#!/usr/bin/env node
// TST-035：建立演練用的隔離 fixture repository（沿用 TST-033 的 1 份 Spec、3 張有依賴的 Story）。
// 用法：node build-fixture.mjs <empty-target-directory> [batch-id]
// AC-006 以第二個 batch-id 另建一個 fixture，使 Goal ID 與主演練不同。
// Sidecar 的兩個 digest 先填 0，由 `review readiness-digests` 更新，以實際走過該命令。
import { execFileSync } from "node:child_process";
import { mkdir, readdir, writeFile } from "node:fs/promises";
import { dirname, join, resolve } from "node:path";

const BATCH_ID = process.argv[3] ?? "BR-935-rehearsal";
...
async function main() {
  ...
  await put(root, "README.md", "# Greeting fixture\n\nRehearsal fixture for PraxisBound TST-035.\n");
  // ForgePilot 的 Runner 在每個 Work Item 之後執行 `make verify`（TST-035 實測發現，TST-033 的 fixture 缺這個檔）。
  await put(root, "Makefile", "verify:\n\tnode --test\n");
  ...
  git("init", "-q", "-b", "main");
  git("config", "user.name", "TST-035 rehearsal");
