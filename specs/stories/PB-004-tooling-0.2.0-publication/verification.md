...
* `AC-008`: pass — `tests/protocol.sh PB004-AC-008 passed on 5c7043d, including full-history checkout for publish.yml and verify.yml; deleting the candidate guard was observed to fail it. After Human Review requested a server-side remote verification guard (#72), the extended case was observed to fail when the verify.yml success comparison was deleted from publish.yml, and to pass once restored.`
