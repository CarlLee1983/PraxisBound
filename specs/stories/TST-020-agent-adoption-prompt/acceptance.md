...
* [ ] AC-003: On the Node fixture with no `AGENTS.md`, one Claude Code and one
  ...
  outside the fixture other than package or build caches and temporary files.
...
* [ ] AC-006: On the Go fixture holding an `AGENTS.md`, one Claude Code and one
  ...
  test is deliberately broken, include the `AGENTS.md` diff in the report, and
  do not commit.
...
* [ ] AC-008: All six observations use the prompt at one recorded commit, each a
  fresh non-interactive session with the prompt as its only input per R8, and an
  observation made on an earlier prompt or with other input is recorded as not
  counting.
...
| `AC-003` | human | `TST-020 agent observation record` | `Node fixture without AGENTS.md; fresh non-interactive Claude Code and Codex sessions; final prompt commit` | `agent-inspected Doctor PASS, verify exit 0, broken-test gate failure, clean commit history and no outside change, for both agents` |
...
| `AC-006` | human | `TST-020 agent observation record` | `Go fixture with marked AGENTS.md; prompt with optional authorization; both agents` | `agent-inspected marked rules and PraxisBound sections present, Doctor PASS, verify exit 0, broken-test gate failure, AGENTS.md diff in the report, no commit, for both agents` |
...
recorded as not counting. Observations run as fresh non-interactive sessions
the agent starts: `claude -p` with a fixed tool allowlist and `codex exec` in
the workspace-write sandbox with network access, never bypassing permissions or
sandboxing. Once the prompt is final and committed, run all six
