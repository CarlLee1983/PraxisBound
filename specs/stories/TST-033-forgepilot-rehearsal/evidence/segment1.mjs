#!/usr/bin/env node
// TST-033：依 contract §11（TST-032 修訂版）執行第一段「建立並預覽」，並以 `review observe` 記錄。
// 用法：node segment1.mjs <config.json> <observation-output.json>
...
if (list.exit !== 0) {
  ...
  // ForgePilot 32b7a68 以大寫 `GOAL` 回報 `--review-policy goal`；contract §11 寫小寫 `goal`（演練發現）。
...
// ForgePilot 32b7a68 在驗證失敗時仍 exit 0，失敗只在 diagnostics（§11 未寫明，列為演練發現）。
if ((goalPreflightResult.diagnostics ?? []).length > 0) finish("goal-preflight-failed");
...
if ((planResult.diagnostics ?? []).length > 0 || typeof planResult.approvalToken !== "string") finish("result-unknown");
