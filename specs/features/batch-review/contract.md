...
- （修訂，R-005）Review Projection 的修訂紀錄證據（§20）：`records/` 內檔名以 `revisions-` 或 `responses-` 開頭、以 `.json` 結尾的檔案（含不合法者）合計 ≤ 200 份；
  所有有效紀錄的意見與回應合計 ≤ 10000 則。超過時整個證據區不呈現任何紀錄內容，不截斷、不部分呈現（§20）。
...
- 先依檔名計數；超過 200 份時不讀取任何紀錄內容。讀取後有效紀錄的意見與回應合計超過 10000 則時，同樣不呈現任何紀錄內容。
...
- 全部紀錄內容（含路徑、ID、診斷）一律以文字節點呈現，控制字元以可見形式跳脫，不成為 markup、屬性、URL、命令或網路請求；§19 的 CSP 不變。
